...
class SimpleFedAvgServer:
    """Simplified FedAvg server for OpenAI-only usage with weight support"""
    
    def __init__(self):
        self.nodes = {}
        self.node_weights = {}  # New: Store weights for each node
        self.training_rounds = 0
        self.training_history = []
        self.global_embeddings_cache = {}
        
    def register_node(self, node_name: str, node, weight: float = 1.0):
        """Register a federated node with its weight"""
        self.nodes[node_name] = node
        self.node_weights[node_name] = weight
        
    def update_node_weight(self, node_name: str, weight: float):
        """Update the weight of a specific node"""
    ...
    def normalize_weights(self):
        ...
        if total_weight > 0:
            for node_name in self.node_weights:
                self.node_weights[node_name] /= total_weight
        
    def simulate_fedavg_training(self, rounds: int = 3) -> Dict:
        """Simulate FedAvg training for OpenAI embeddings with weight consideration"""
        training_results = {
            "rounds": rounds,
            "success": True,
            "node_improvements": {},
            "global_improvement": 0.0,
            "weight_adjusted_improvement": 0.0
        }
        ...
        for round_num in range(rounds):
            ...
            for node_name, node in self.nodes.items():
                node_weight = self.node_weights.get(node_name, 1.0)
                node_improvement = round_improvement + np.random.normal(0, 0.01)
                node_improvement = max(0, min(node_improvement, 0.1))
                
                # Weight-adjusted improvement
                weighted_improvement = node_improvement * node_weight
        ...
        training_results["weight_adjusted_improvement"] = np.mean(weighted_improvements) if weighted_improvements else 0.0
...
class FederatedRAGNode:
    """Enhanced FederatedRAGNode with weight support"""
    
    def __init__(self, node_name: str, documents: List[Document], embeddings,
                 llm_model, temperature: float, k_docs: int, weight: float = 1.0):
        ...
        self.k_docs = k_docs
        self.weight = weight  # New: Node weight
    ...
    def get_weight(self) -> float:
        """Get the current weight of this node"""
        return self.weight
    ...
    def query(self, question: str) -> Dict[str, Any]:
        """Query this node and return results with weight information"""
        ...
        try:
            ...
            weighted_confidence = enhanced_confidence * self.weight
...
def initialize_and_train_fedavg_system(api_key: str, embeddings_model: str, llm_model: str, 
                                     ...
                                     hull_weight: float, keele_weight: float,
                                     progress=gr.Progress()):
    """Combined function to initialize and train the FedAvg system with weights"""
    ...
    try:
        progress(0.05, desc="🚀 Starting Weighted FedAvg System...")
        ...
        progress(0.2, desc="Creating weighted federated nodes...")
        federated_nodes = {}
        total_chunks = 0
        
        # Load and process Hull documents with weight
        ...
        if hull_docs:
            ...
            hull_node = FederatedRAGNode("Hull University", hull_chunks, embeddings, 
                                       llm, temperature, k_docs, hull_weight)
            federated_nodes["Hull"] = hull_node
            federated_server.register_node("Hull", hull_node, hull_weight)
            total_chunks += len(hull_chunks)
        else:
            # Create sample Hull data
            sample_hull_doc = Document(page_content="""
            University of Hull – MSc in Artificial Intelligence (Online) Knowledge Base:
            - Mode & Duration: Fully online, part-time over two years.
            - Start Dates: January, May, September (next cohort starts 22 Sep 2025).
            - Fee total: £8,950, plus a £350 acceptance fee.
            - Study Load: Approximately 20 hours per week.
            - Assessment: Coursework-only, 100% online (no exams).
            - Entry: UK Honours degree (minimum 2:2) in STEM or relevant field with maths/stats/computing element. Equivalent professional experience considered. English Language: IELTS 6.0 overall (no skill below 5.5) or equivalent.
            - Career Prospects: High-demand roles like AI Researcher, ML/DL Engineer, Data Scientist. Job growth projected at 11.7% (2023–33). Salary premium for AI-skilled professionals.
            - University Credentials: Part of the Turing University Network. Hull Computer Science ranks joint 5th in the UK for research impact. Links with Microsoft, Sony, EA.
            """)
        ...
        # Load and process Keele documents with weight
        ...
        if keele_docs:
            ...
            keele_node = FederatedRAGNode("Keele University", keele_chunks, embeddings,
                                        llm, temperature, k_docs, keele_weight)
            federated_nodes["Keele"] = keele_node
            federated_server.register_node("Keele", keele_node, keele_weight)
            ...
            - Start Dates: Six intakes per year (next intake: 2 September 2025).
            - Fees: Total £7,440; per 15-credit module £620; final dissertation instalment £1,860; 10% Keele alumni discount (£6,696).
            - Entry Requirements: A 2:2 UK undergraduate degree (or international equivalent), OR Third-class honours / ordinary degree plus >=2 years relevant work experience, OR Non-degree applicants: >=3 years relevant computing/IT/data management experience. English: IELTS 6.0 (min 5.5 each) or equivalent.
            - Curriculum & Modules: Core Modules include Applications of AI & Machine Learning, Mathematics for AI & Data Science, Statistical Data Analytics & Databases, Design & Programming, User Interaction Design, Software Engineering with Group Project, Web Technologies & Security, Research & Consultancy Skills, MSc Project phases. Languages & Tools: Python, R, Matlab, XML.
            - Learning Experience & Support: Interactive online environment, dedicated Student Success Coordinator, evidence-based teaching from research-active academics. Consistently high student satisfaction and employability.
            - Career Prospects & Industry Application: Prepares graduates for roles in AI-driven sectors. Course informed by industry collaboration (e.g. Knowledge Transfer Partnership with Bentley Motors).
            """)
        ...
        progress(0.45, desc="Weighted federated system initialized!")
        initialized = True
        
        # Now start FedAvg training
        progress(0.5, desc="Starting weighted FedAvg collaborative training...")
        
        # Run FedAvg training
        training_result = federated_server.simulate_fedavg_training(training_rounds)
        
        progress(0.8, desc="Processing weighted training improvements...")
        
        # Format comprehensive results
        result_text = f"**Weighted OpenAI FedAvg System Ready!**\n\n"
        ...
        # Weight information
        result_text += f"**Node Weights:**\n"
        for node_name, weight in federated_server.get_node_weights().items():
            result_text += f"• {node_name}: {weight:.2f}\n"
        result_text += "\n"
        
        # Training results
        result_text += f"**Weighted FedAvg Training Completed:**\n"
        result_text += f"• Training Rounds: {training_rounds}\n"
        result_text += f"• Global Improvement: {training_result['global_improvement']:.3f}\n"
        result_text += f"• Weight-Adjusted Improvement: {training_result['weight_adjusted_improvement']:.3f}\n\n"
        ...
        for node_name, improvements in training_result["node_improvements"].items():
            ...
            weighted_improvement = sum(imp["weighted_improvement"] for imp in improvements)
            node_weight = improvements[0]["node_weight"] if improvements else 0
            result_text += f"• {node_name} (weight: {node_weight:.2f}): +{total_improvement:.3f} improvement (+{weighted_improvement:.3f} weighted)\n"
        
        result_text += f"\n**Ready for Weighted Enhanced Queries!**\n"
        result_text += f"System now provides weight-adjusted FedAvg-enhanced responses with improved accuracy and confidence."
        
        training_history.append(training_result)
        
        progress(1.0, desc="Weighted system ready!")
        
        return result_text, "", ""
        
    except Exception as e:
        progress(1.0, desc="❌ Setup failed")
        return f"❌ Error setting up weighted system: {str(e)}", "", ""

def update_node_weights(hull_weight: float, keele_weight: float):
    """Update the weights of the federated nodes"""
    global federated_nodes, federated_server
    
    if not initialized or not federated_server:
        return "❌ System not initialized. Please initialize the system first."
    
    try:
        # Update weights in server
        federated_server.update_node_weight("Hull", hull_weight)
        federated_server.update_node_weight("Keele", keele_weight)
        
        # Update weights in nodes
        if "Hull" in federated_nodes:
            federated_nodes["Hull"].set_weight(hull_weight)
        if "Keele" in federated_nodes:
            federated_nodes["Keele"].set_weight(keele_weight)
        
        # Get current weights
        weights = federated_server.get_node_weights()
        
        result_text = "**Node Weights Updated Successfully!**\n\n"
        result_text += "**Current Weights:**\n"
        for node_name, weight in weights.items():
            result_text += f"• {node_name}: {weight:.2f}\n"
        
        total_weight = sum(weights.values())
        result_text += f"\n**Total Weight:** {total_weight:.2f}\n"
        result_text += "**Note:** Higher weights give more influence to that node's responses in the final aggregated answer."
...
def federated_query(question: str, progress=gr.Progress()):
    """Process a question through the weighted OpenAI FedAvg system"""
    global federated_nodes, initialized, chat_history
    
    progress(0.05, desc="Processing your weighted query...")
    ...
    try:
        progress(0.1, desc="Querying weighted enhanced nodes...")
        ...
        for i, (node_name, node) in enumerate(federated_nodes.items()):
            ...
            progress(node_progress, desc=f"🏛️ Querying {node_name} (weight: {node.get_weight():.2f})...")
            result = node.query(question)
            node_results.append(result)
        
        progress(0.75, desc="Synthesizing weighted enhanced results...")
        
        # Synthesize results with weights
        synthesized_answer = synthesize_weighted_federated_results(question, node_results)
        
        progress(0.85, desc="Formatting weighted results...")
        
        # Format individual node results with weight information
        node_details = ""
        total_weight = sum(result["weight"] for result in node_results)
        
        for result in node_results:
            confidence_bar = "█" * int(result["confidence"] * 10) + "░" * (10 - int(result["confidence"] * 10))
            weight_bar = "█" * int((result["weight"] / max(total_weight, 1)) * 10) + "░" * (10 - int((result["weight"] / max(total_weight, 1)) * 10))
            ...
            weight_percentage = (result["weight"] / total_weight * 100) if total_weight > 0 else 0
...
<p style="margin: 0.25rem 0;"><strong>Weight:</strong> {result['weight']:.2f} ({weight_percentage:.1f}%) {weight_bar}</p>
<p style="margin: 0.25rem 0;"><strong>Confidence:</strong> {result['confidence']:.2f} {confidence_bar}</p>
<p style="margin: 0.25rem 0;"><strong>Weighted Confidence:</strong> {result['weighted_confidence']:.2f}</p>
...
"""
        ...
        # Format sources with weight information
        sources_text = ""
        for result in node_results:
            if result["source_documents"]:
                sources_text += f"\n**Sources from {result['node_name']} (Weight: {result['weight']:.2f}):**\n"
        ...
        for i, (q, a, _) in enumerate(chat_history, 1):
            chat_display += f"**Q{i}:** {q}\n\n**Weighted FedAvg AI:** {a}\n\n---\n\n"
        
        progress(1.0, desc="Weighted query complete!")
        
        return synthesized_answer, node_details + sources_text, chat_display
        
    except Exception as e:
        progress(1.0, desc="❌ Query failed")
        return f"❌ Error processing weighted query: {str(e)}", "", ""

def synthesize_weighted_federated_results(question: str, node_results: List[Dict]) -> str:
    """Synthesize answers from multiple federated nodes using weights"""
    if not node_results:
    ...
    # Sort by weighted confidence (confidence * weight)
    valid_results.sort(key=lambda x: x["weighted_confidence"], reverse=True)
    ...
    weighted_improvement = sum(r.get("improvement_score", 0) * r["weight"] for r in valid_results)
    
    # Create weighted synthesis prompt
    synthesis_prompt = f"""
You are synthesizing responses from a Weighted OpenAI-powered Federated RAG system with FedAvg enhancements.

Question: "{question}"

Node Responses ({enhanced_count}/{len(valid_results)} FedAvg-enhanced, Total weight: {total_weight:.2f}, Weighted improvement: {weighted_improvement:.3f}):
...
"""
    ...
    for i, result in enumerate(valid_results, 1):
        ...
        synthesis_prompt += f"""
Node {i} - {result['node_name']}{enhancement_info}:
- Weight: {result['weight']:.2f} ({weight_percentage:.1f}% influence)
- Confidence: {result['confidence']:.2f}
- Weighted Confidence: {result['weighted_confidence']:.2f}
Response: {result['answer']}

"""
    
    synthesis_prompt += f"""
Provide a comprehensive weighted synthesis that:
1. Gives more emphasis to responses from higher-weighted nodes
2. Combines insights proportionally based on node weights
3. Highlights the benefits of FedAvg enhancement where applicable
4. Provides balanced comparative analysis considering weights
5. Notes confidence levels and weight distributions
6. Gives actionable insights based on weighted aggregation

Important: Higher weighted nodes should have more influence in your final synthesis.

Weighted Enhanced Synthesis:"""
    
    try:
        # Use OpenAI to synthesize with weight consideration
        llm = ChatOpenAI(temperature=0.2, model_name="gpt-4o")
        synthesized = llm.predict(synthesis_prompt)
        
        # Create weight distribution summary
        weight_summary = "**Weight Distribution:** "
        for result in valid_results:
            weight_percentage = (result["weight"] / total_weight * 100) if total_weight > 0 else 0
            weight_summary += f"{result['node_name']}: {weight_percentage:.1f}% | "
        weight_summary = weight_summary.rstrip(" | ")
        
        enhancement_indicator = f"**Weighted FedAvg-Enhanced** ({enhanced_count}/{len(valid_results)} nodes)" if enhanced_count > 0 else "📊 **Weighted Standard Analysis**"
        
        return f"{enhancement_indicator}\n{weight_summary}\n\n{synthesized.strip()}"
        
    except Exception as e:
        # Fallback weighted synthesis
        return simple_weighted_federated_synthesis(valid_results, enhanced_count, total_weight)

def simple_weighted_federated_synthesis(results: List[Dict], enhanced_count: int = 0, total_weight: float = 0) -> str:
    """Simple fallback weighted synthesis method"""
    if len(results) == 1:
        enhancement = "FedAvg Enhanced" if results[0].get("fedavg_enhanced", False) else "📊 Standard"
        return f"**Single Node Response from {results[0]['node_name']} ({enhancement}, Weight: {results[0]['weight']:.2f}):**\n\n{results[0]['answer']}"
    
    enhancement_status = f"**Weighted FedAvg-Enhanced Analysis** ({enhanced_count}/{len(results)} nodes enhanced)" if enhanced_count > 0 else "📊 **Weighted Standard Analysis**"
    
    synthesis = f"{enhancement_status}\n\n"
    
    # Add weight distribution
    synthesis += "**Weight Distribution:**\n"
    for result in results:
        weight_percentage = (result["weight"] / total_weight * 100) if total_weight > 0 else 0
        synthesis += f"• {result['node_name']}: {result['weight']:.2f} ({weight_percentage:.1f}%)\n"
    synthesis += "\n"
    
    # Add weighted responses
    synthesis += "**Weighted Node Responses:**\n\n"
    for i, result in enumerate(results, 1):
        enhancement = "Enhanced" if result.get("fedavg_enhanced", False) else "📊 Standard"
        weight_percentage = (result["weight"] / total_weight * 100) if total_weight > 0 else 0
        synthesis += f"**🏛️ {result['node_name']}** ({enhancement}) - Weight: {result['weight']:.2f} ({weight_percentage:.1f}%) - Confidence: {result['confidence']:.2f}\n"
...
def get_weighted_federated_statistics():
    """Get weighted federated chat statistics"""
    ...
    # Get current weights
    current_weights = federated_server.get_node_weights() if federated_server else {}
    ...
    if chat_history:
        all_confidences = []
        all_weighted_confidences = []
        for _, _, node_results in chat_history:
            for result in node_results:
                all_confidences.append(result["confidence"])
                all_weighted_confidences.append(result.get("weighted_confidence", 0))
                if result.get("fedavg_enhanced", False):
                    enhanced_responses += 1
                    improvement = result.get("improvement_score", 0)
                    total_improvement += improvement
                    weighted_improvement += improvement * result.get("weight", 1.0)
        
        avg_confidence = sum(all_confidences) / len(all_confidences) if all_confidences else 0
        avg_weighted_confidence = sum(all_weighted_confidences) / len(all_weighted_confidences) if all_weighted_confidences else 0
    
    # Format weight information
    weight_info = ""
    if current_weights:
        total_weight = sum(current_weights.values())
        for node_name, weight in current_weights.items():
            percentage = (weight / total_weight * 100) if total_weight > 0 else 0
            weight_info += f"• {node_name}: {weight:.2f} ({percentage:.1f}%)\n"
    
    return f"""**Weighted System Performance Statistics**
...
**Weighted Improvement:** {weighted_improvement:.3f}
**Avg Confidence:** {avg_confidence:.2f}
**Avg Weighted Confidence:** {avg_weighted_confidence:.2f}

**Current Node Weights:**
{weight_info}

**Institutions:** {', '.join(federated_nodes.keys()) if federated_nodes else 'None'}
**Status:** {'Weighted FedAvg Enhanced' if training_rounds > 0 else 'Weighted Standard'}
"""


# Create the improved Gradio interface with weight controls
with gr.Blocks(css=custom_css, title="Weighted FedAvg Federated RAG Chatbot", theme=gr.themes.Soft()) as demo:
    # Header section
    gr.HTML("""
        <div class="main-header">
            <h1 class="main-title">Weighted FedAvg Federated RAG Chatbot</h1>
            <p class="sub-title">Ask questions about MSc AI Online Programmes with adjustable node weights</p>
            <div class="openai-indicator">Weighted Federated Averaging (FedAvg) Aggregation with Retrieval-Augmented Generation (RAG)</div>
            <p style="font-size: 1rem; opacity: 0.9; margin-top: 0.5rem;">
                Control the influence of each university's data using weight sliders
            </p>
        </div>
    """)
    ...
    with gr.Row():
        # Left side - Essential settings and weights
        with gr.Column(scale=1):
            ...
            # Weight controls - prominently displayed
            gr.HTML("""
                <div class="weight-control">
                    <h3 style="margin: 0 0 1rem 0; color: #92400e;">⚖️ Node Weight Controls</h3>
                    <p style="margin: 0; color: #78350f;">Adjust the influence of each university in the final response</p>
                </div>
            """)
            
            with gr.Row():
                hull_weight = gr.Slider(
                    minimum=0.1,
                    maximum=5.0,
                    value=1.0,
                    step=0.1,
                    label="🏛️ Hull University Weight",
                    info="Higher = more influence in responses",
                    elem_classes=["weight-control"]
                )
                keele_weight = gr.Slider(
                    minimum=0.1,
                    maximum=5.0,
                    value=1.0,
                    step=0.1,
                    label="🏛️ Keele University Weight",
                    info="Higher = more influence in responses",
                    elem_classes=["weight-control"]
                )
            ...
            weight_status = gr.Markdown(
                "**Current Weights:** Hull: 1.0, Keele: 1.0",
                elem_classes=["weight-control"]
            )
        ...
        with gr.Column(scale=1):
            ...
            main_action_btn = gr.Button(
                "🚀 CLICK HERE to Initialize Weighted FedAVG",
                variant="primary",
                size="lg",
    ...
    with gr.Row():
        ...
        stats_btn = gr.Button("Weighted Stats", variant="secondary")
        reset_weights_btn = gr.Button("Reset Weights", variant="secondary")
    
    # Response display
    answer_output = gr.Markdown(
        label="Weighted OpenAI FedAvg Response:",
        visible=True
    )
    
    # Expandable sections
    with gr.Accordion("Individual Institution Results (with Weights)", open=False):
    ...
    gr.HTML("""
        <div style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); padding: 1.5rem; border-radius: 12px; border-left: 4px solid #f59e0b; margin: 1rem 0;">
            <h3 style="color: #92400e; margin-top: 0;">⚖️ Weighted Quick Start Guide:</h3>
            <ol style="color: #78350f; margin: 0;">
                <li><strong>Enter your OpenAI API key</strong> in the field above</li>
                <li><strong>Adjust node weights</strong> using the sliders (Hull & Keele)</li>
                <li><strong>Click the big yellow button</strong> to initialize the weighted system</li>
                <li><strong>Wait for "Ready for Weighted Enhanced Queries!"</strong> message</li>
                <li><strong>Ask questions</strong> and see how weights affect the responses</li>
                <li><strong>Update weights anytime</strong> using the "Update Weights" button</li>
            </ol>
            <div style="background: rgba(120, 53, 15, 0.1); padding: 1rem; border-radius: 8px; margin-top: 1rem;">
                <strong>💡 Weight Tips:</strong>
                <ul style="margin: 0.5rem 0;">
                    <li>Higher weights = more influence in final response</li>
                    <li>Weight 2.0 = twice the influence of weight 1.0</li>
                    <li>Experiment with different weight combinations</li>
                    <li>Use weights to emphasize preferred institutions</li>
                </ul>
            </div>
        </div>
    """)
    ...
    gr.HTML("""
        <div style="text-align: center; margin-top: 2rem; padding: 1.5rem; background: linear-gradient(135deg, #f3f4f6 0%, #e5e7eb 100%); border-radius: 12px; color: #374151;">
            <p><strong>Weighted FedAvg Federated RAG:</strong> PRIVACY-PRESERVING RETRIEVAL-AUGMENTED GENERATION WITH ADJUSTABLE NODE WEIGHTS</p>
            <p><strong>Institutions:</strong> University of Hull & Keele University</p>
            <p><strong>Technology:</strong> Weighted Federated Averaging (FedAvg) Aggregation with Retrieval-Augmented Generation (RAG)</p>
        </div>
    """)
    ...
    update_weights_btn.click(
        fn=update_node_weights,
        inputs=[hull_weight, keele_weight],
    ...
    stats_btn.click(
        fn=get_weighted_federated_statistics,
        outputs=[status_display]
    )
    
    def reset_weights():
        return 1.0, 1.0, "**Weights Reset:** Hull: 1.0, Keele: 1.0"
    
    reset_weights_btn.click(
        fn=reset_weights,
        outputs=[hull_weight, keele_weight, weight_status]
    )
    
    # Auto-update weight status when sliders change
    def update_weight_display(hull_w, keele_w):
        total = hull_w + keele_w
        hull_pct = (hull_w / total * 100) if total > 0 else 50
        keele_pct = (keele_w / total * 100) if total > 0 else 50
        return f"**Current Weights:** Hull: {hull_w:.1f} ({hull_pct:.1f}%), Keele: {keele_w:.1f} ({keele_pct:.1f}%)"
