...
def create_llm(provider: str, llm_model: str, temperature: float, api_key: str = None):
    ...
    if provider == "OpenAI":
        ...
        # Use better HuggingFace models with higher token limits
        try:
            ...
            try:
                ...
                pipe = pipeline(
                    ...
                    model="distilgpt2",
                    tokenizer=tokenizer,
                    max_new_tokens=256,  # Increased from 128 to 256
                    temperature=temperature,
                    do_sample=True if temperature > 0 else False,
                    pad_token_id=tokenizer.eos_token_id,
                    truncation=True
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            provider_dropdown = gr.Dropdown(
                ...
                label="Provider", 
                value="OpenAI",
                info="Choose OpenAI (requires API key) or HuggingFace (free, no API key needed)"
            )
