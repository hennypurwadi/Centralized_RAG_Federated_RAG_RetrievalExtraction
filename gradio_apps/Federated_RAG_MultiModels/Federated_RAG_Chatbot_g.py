...
def create_llm(provider: str, llm_model: str, temperature: float, api_key: str = None):
    ...
    if provider == "OpenAI":
        ...
        # Use simple HuggingFace pipeline for free models
        try:
...
def initialize_federated_rag_system(provider: str, api_key: str, embeddings_model: str, llm_model: str, 
    ...
    try:
        ...
        progress(0.25, desc="🏗️ Preparing federated nodes...")
        federated_nodes = {}
        total_chunks = 0
        ...
        if hull_docs:
            progress(0.4, desc="✂️ Chunking Hull documents...")
            hull_chunks = chunk_documents(hull_docs, chunk_size, chunk_overlap)
            progress(0.5, desc="🏛️ Creating Hull University node...")
        ...
        if keele_docs:
            progress(0.7, desc="✂️ Chunking Keele documents...")
            keele_chunks = chunk_documents(keele_docs, chunk_size, chunk_overlap)
            progress(0.8, desc="🏛️ Creating Keele University node...")
...
def federated_query(question: str, progress=gr.Progress()):
    ...
    try:
        ...
        progress(0.15, desc="🏛️ Connecting to federated nodes...")
        ...
        for result in node_results:
            ...
            node_details += f"""
**🏛️ {result['node_name']}** (Confidence: {result['confidence']:.2f})
{confidence_bar}
...
🏛️ **Participating Institutions:** {', '.join(federated_nodes.keys())}
⚡ **Provider:** {provider}
"""
...
def update_models_based_on_embeddings_provider(provider):
    ...
    if provider == "OpenAI":
        embeddings_choices = ["text-embedding-3-small", "text-embedding-3-large", "text-embedding-ada-002"]
...
with gr.Blocks(css=custom_css, title="Federated RAG Chatbot", theme=gr.themes.Soft()) as demo:
    ...
    gr.HTML("""
        <div class="description">
            ...
            <p><strong>🆓 Free Options:</strong> Choose between OpenAI (requires API key) or Huggingface (completely free) models for embeddings and language generation.</p>
        </div>
    """)
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            provider = gr.Dropdown(
                ...
                info="Choose between OpenAI (paid) or Huggingface (free) - LLM will follow this selection"
            )
